...
export type AdminUserAuditPayload = { full_name?: string | null; can_sales?: boolean; can_catalog_inventory?: boolean; can_ledger?: boolean; can_cheques_reports?: boolean; reason: string; expected_updated_at?: string };
...
export const api = {
  ...
  createUser(payload: { username: string; full_name: string; temporary_password: string; can_sales: boolean; can_catalog_inventory: boolean; can_ledger: boolean; can_cheques_reports: boolean; reason: string }) { return request<User>("/users", { method: "POST", body: JSON.stringify(payload) }); },
  ...
  chequePersonOptions() {
    return request<Person[]>("/cheques/form-options");
  },
