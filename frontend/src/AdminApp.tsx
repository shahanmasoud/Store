...
function AdminApp({ onOpenStore }: { onOpenStore: () => void }) {
  ...
  function navigateView(target: AppView) {
    const catalogView = target === "products" || target === "purchase" || target === "inventory";
    const chequeReportView = target === "cheques" || target === "reports";
    const allowed = user?.is_superuser || target === "dashboard" || (target === "sales" && user?.can_sales) || (catalogView && user?.can_catalog_inventory) || (target === "ledger" && user?.can_ledger) || (chequeReportView && user?.can_cheques_reports);
  ...
  function renderNavigation() {
    return (
      <nav className="sidebar-nav" aria-label="بخش‌های پنل مدیریت">
        ...
        {moduleCards.filter((card) => user?.is_superuser || (card.key === "sales" && user?.can_sales) || (["products", "purchase", "inventory"].includes(card.key) && user?.can_catalog_inventory) || (card.key === "ledger" && user?.can_ledger) || (["cheques", "reports"].includes(card.key) && user?.can_cheques_reports)).map((card) => {
  ...
  return (
    <main className="app-layout">
      ...
      {view === "cheques" && (user?.is_superuser || user?.can_cheques_reports) ? <ChequesView onBack={() => navigateView("dashboard")} onOpenLedger={() => navigateView("ledger")} /> : null}
...
function ChequesView({ onBack, onOpenLedger }: { onBack: () => void; onOpenLedger: () => void }) {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down("sm"));
  const [people, setPeople] = useState<Person[]>([]);
