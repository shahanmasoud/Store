...
function AdminApp({ onOpenStore }: { onOpenStore: () => void }) {
  ...
  function navigateView(target: AppView) {
    ...
    const allowed = user?.is_superuser || target === "dashboard" || (target === "sales" && user?.can_sales) || (catalogView && user?.can_catalog_inventory);
  ...
  function renderNavigation() {
    return (
      <nav className="sidebar-nav" aria-label="بخش‌های پنل مدیریت">
        ...
        {moduleCards.filter((card) => user?.is_superuser || (card.key === "sales" && user?.can_sales) || (["products", "purchase", "inventory"].includes(card.key) && user?.can_catalog_inventory)).map((card) => {
  ...
  return (
    <main className="app-layout">
      ...
      {view === "dashboard" ? (
        ...
        /> : user ? <RoleHome user={user} onOpenSales={() => navigateView("sales")} onOpenProducts={() => navigateView("products")} onOpenPurchase={() => navigateView("purchase")} onOpenInventory={() => navigateView("inventory")} /> : null
      ) : null}
      </div>
    </main>
  );
...
function RoleHome({ user, onOpenSales, onOpenProducts, onOpenPurchase, onOpenInventory }: { user: User; onOpenSales: () => void; onOpenProducts: () => void; onOpenPurchase: () => void; onOpenInventory: () => void }) {
...
function UsersView({ onBack, currentUserId }: { onBack: () => void; currentUserId: number | string }) {
  ...
  const [username, setUsername] = useState(""); const [fullName, setFullName] = useState(""); const [password, setPassword] = useState(""); const [canSales, setCanSales] = useState(true); const [canCatalog, setCanCatalog] = useState(false); const [createReason, setCreateReason] = useState(""); const [saving, setSaving] = useState(false);
  const [editing, setEditing] = useState<User | null>(null); const [editName, setEditName] = useState(""); const [editSales, setEditSales] = useState(false); const [editCatalog, setEditCatalog] = useState(false); const [editReason, setEditReason] = useState(""); const [dialogError, setDialogError] = useState("");
