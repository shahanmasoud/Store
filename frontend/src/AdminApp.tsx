...
function jalaliDateAfterDays(days: number) {
  const date = new Date();
  date.setDate(date.getDate() + days);
  const parts = new Intl.DateTimeFormat("fa-IR-u-ca-persian", { timeZone: "Asia/Tehran", year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(date);
  const part = (type: Intl.DateTimeFormatPartTypes) => toEnglishDigits(parts.find((item) => item.type === type)?.value ?? "");
  return `${part("year")}/${part("month").padStart(2, "0")}/${part("day").padStart(2, "0")}`;
}
...
function JalaliDateField({
  ...
  return (
    <label className="date-field">
      {label}
      <button type="button" className="date-trigger" aria-expanded={open} onClick={() => setOpen((current) => !current)} disabled={disabled}>
      ...
      {open ? (
        <div className="date-popover" role="dialog" aria-label="انتخاب تاریخ شمسی">
          <div className="date-controls">
            <select value={selected.year} onChange={(event) => updateDate({ year: Number(event.target.value) })} disabled={disabled}>
              {years.map((year) => (
            ...
            <select value={selected.month} onChange={(event) => updateDate({ month: Number(event.target.value) })} disabled={disabled}>
          ...
          {!required && value ? <Button size="small" color="inherit" onClick={() => { onChange(""); setOpen(false); }} disabled={disabled}>حذف تاریخ</Button> : null}
        </div>
      ) : null}
    </label>
  );
...
function AdminApp({ onOpenStore }: { onOpenStore: () => void }) {
  ...
  function navigateView(target: AppView) {
    const allowed = canAccessAdminView(user, target);
...
function RoleHome({ user, onOpenSales, onOpenProducts, onOpenPurchase, onOpenInventory, onOpenLedger, onOpenCheques, onOpenReports }: { user: User; onOpenSales: () => void; onOpenProducts: () => void; onOpenPurchase: () => void; onOpenInventory: () => void; onOpenLedger: () => void; onOpenCheques: () => void; onOpenReports: () => void }) {
  const [inventory, setInventory] = useState<InventoryReport | null>(null); const [inventoryError, setInventoryError] = useState("");
  const [ledgerReminderText, setLedgerReminderText] = useState("در حال دریافت سررسیدها…");
  useEffect(() => { if (!user.can_catalog_inventory) return; api.inventoryReport().then(setInventory).catch((error) => setInventoryError(error instanceof Error ? error.message : "خلاصه انبار دریافت نشد.")); }, [user.can_catalog_inventory]);
  useEffect(() => { if (!user.can_ledger) return; const today = currentJalaliDate(); api.dueReminders(today, jalaliDateAfterDays(7)).then((result) => setLedgerReminderText(`${toPersianDigits((result.overdue.count + result.today.count).toLocaleString("fa-IR"))} مورد نیازمند پیگیری`)).catch((error) => setLedgerReminderText(error instanceof Error ? error.message : "سررسیدها دریافت نشدند.")); }, [user.can_ledger]);
  const cards = [user.can_sales ? { title: "ثبت فروش", text: "فاکتور جدید و روش پرداخت", icon: <PointOfSaleRounded />, action: onOpenSales } : null, user.can_catalog_inventory ? { title: "کالاها", text: "تعریف و ویرایش اطلاعات کالا", icon: <Inventory2Rounded />, action: onOpenProducts } : null, user.can_catalog_inventory ? { title: "ثبت خرید", text: "ورود خرید و افزایش موجودی", icon: <ShoppingCartCheckoutRounded />, action: onOpenPurchase } : null, user.can_catalog_inventory ? { title: "انبار", text: inventory ? `${toPersianDigits(inventory.low_stock_count.toLocaleString("fa-IR"))} کالای کم‌موجود` : inventoryError || "در حال دریافت وضعیت انبار…", icon: <FactCheckRounded />, action: onOpenInventory } : null, user.can_ledger ? { title: "دفتر حساب", text: ledgerReminderText, icon: <AccountBalanceWalletRounded />, action: onOpenLedger } : null, user.can_cheques_reports ? { title: "چک‌ها", text: "ثبت و پیگیری چک‌های دریافتی و پرداختی", icon: <PaymentsRounded />, action: onOpenCheques } : null, user.can_cheques_reports ? { title: "گزارش‌ها", text: "فروش، سود، نقدینگی و بدهی مشتریان", icon: <AnalyticsRounded />, action: onOpenReports } : null].filter(Boolean) as Array<{ title: string; text: string; icon: ReactNode; action: () => void }>;
  return <Box className="cashier-home"><Alert severity="info">فقط بخش‌های لازم برای نقش شما نمایش داده می‌شوند.</Alert><Box className="role-home-cards">{cards.map((card) => <Card className="cashier-sale-card" key={card.title}><CardActionArea onClick={card.action}><CardContent>{card.icon}<Typography variant="h5">{card.title}</Typography><Typography color="text.secondary">{card.text}</Typography><Box component="span" sx={{ display: "inline-flex", alignItems: "center", justifyContent: "center", minHeight: 44, px: 2.5, borderRadius: 2, bgcolor: "primary.main", color: "primary.contrastText", fontWeight: 800 }}>ورود به بخش</Box></CardContent></CardActionArea></Card>)}</Box></Box>;
}
...
function UsersView({ onBack, currentUserId }: { onBack: () => void; currentUserId: number | string }) {
  ...
  const [username, setUsername] = useState(""); const [fullName, setFullName] = useState(""); const [password, setPassword] = useState(""); const [canSales, setCanSales] = useState(true); const [canCatalog, setCanCatalog] = useState(false); const [canLedger, setCanLedger] = useState(false); const [canChequesReports, setCanChequesReports] = useState(false); const [createReason, setCreateReason] = useState(""); const [saving, setSaving] = useState(false);
  const [editing, setEditing] = useState<User | null>(null); const [editName, setEditName] = useState(""); const [editSales, setEditSales] = useState(false); const [editCatalog, setEditCatalog] = useState(false); const [editLedger, setEditLedger] = useState(false); const [editChequesReports, setEditChequesReports] = useState(false); const [editReason, setEditReason] = useState(""); const [dialogError, setDialogError] = useState("");
  ...
  return <CrudWorkspace title="کاربران و دسترسی‌ها" eyebrow="مدیر اصلی" onBack={onBack}>
    ...
    <form className="sale-panel user-create-form" onSubmit={create} noValidate><div className="ledger-section-heading"><div><strong>ساخت حساب زیرمدیریت</strong><small>نام کاربری بعد از ساخت قابل تغییر نیست.</small></div></div><div className="user-form-grid"><TextField label="نام کاربری" value={username} onChange={(e) => setUsername(e.target.value)} required disabled={saving} slotProps={{ htmlInput: { dir: "ltr", minLength: 3, maxLength: 50, autoComplete: "off" } }} /><TextField label="نام و نام خانوادگی" value={fullName} onChange={(e) => setFullName(e.target.value)} required disabled={saving} /><TextField type="password" label="رمز موقت" value={password} onChange={(e) => setPassword(e.target.value)} required disabled={saving} helperText="حداقل ۸ کاراکتر؛ پس از ثبت دوباره نمایش داده نمی‌شود." slotProps={{ htmlInput: { minLength: 8, maxLength: 128, autoComplete: "new-password" } }} /><TextField label="دلیل ایجاد حساب" value={createReason} onChange={(e) => setCreateReason(e.target.value)} required disabled={saving} helperText="حداقل ۳ کاراکتر" /></div><div className="user-permission-switches"><FormControlLabel control={<Switch checked={canSales} onChange={(e) => setCanSales(e.target.checked)} disabled={saving} />} label="ثبت فروش — ساخت فاکتور و دریافت وجه" /><FormControlLabel control={<Switch checked={canCatalog} onChange={(e) => setCanCatalog(e.target.checked)} disabled={saving} />} label="کالا، خرید و انبار — تعریف کالا، ورود خرید و اصلاح موجودی" /><FormControlLabel control={<Switch checked={canLedger} onChange={(e) => setCanLedger(e.target.checked)} disabled={saving} />} label="اشخاص و دفتر حساب — ثبت سند، تسویه و پیگیری سررسید" /><FormControlLabel control={<Switch checked={canChequesReports} onChange={(e) => setCanChequesReports(e.target.checked)} disabled={saving} />} label="چک‌ها و گزارش‌ها — ثبت و پیگیری چک و مشاهده گزارش‌های مدیریتی" /></div><Button type="submit" variant="contained" size="large" disabled={saving || username.trim().length < 3 || !fullName.trim() || password.length < 8 || createReason.trim().length < 3} startIcon={saving ? <CircularProgress size={18} color="inherit" /> : <AddRounded />}>ساخت کاربر</Button></form>
    ...
    <div className="user-card-list">{users.map((item) => <Card key={item.id} className="admin-user-card"><CardContent><div><span><strong>{item.full_name || item.username}</strong><small dir="ltr">@{item.username}</small></span><Chip label={item.is_superuser ? "مدیر اصلی" : item.is_active ? "فعال" : "غیرفعال"} color={item.is_superuser ? "primary" : item.is_active ? "success" : "default"} /></div><p>{item.is_superuser ? "دسترسی کامل" : [item.can_sales ? "فروش" : "", item.can_catalog_inventory ? "کالا، خرید و انبار" : "", item.can_ledger ? "اشخاص و دفتر حساب" : "", item.can_cheques_reports ? "چک‌ها و گزارش‌ها" : ""].filter(Boolean).join(" • ") || "بدون دسترسی عملیاتی"}</p>{!item.is_superuser ? <div className="admin-user-actions"><Button startIcon={<EditRounded />} onClick={() => { setEditing(item); setEditName(item.full_name || ""); setEditSales(Boolean(item.can_sales)); setEditCatalog(Boolean(item.can_catalog_inventory)); setEditLedger(Boolean(item.can_ledger)); setEditChequesReports(Boolean(item.can_cheques_reports)); setEditReason(""); setDialogError(""); }}>ویرایش</Button><Button startIcon={<LockResetRounded />} onClick={() => { setResetting(item); setResetPassword(""); setResetReason(""); setDialogError(""); }}>رمز موقت جدید</Button><Button color="error" disabled={String(item.id) === String(currentUserId) || !item.is_active} onClick={() => { setDeactivating(item); setDeactivateReason(""); setDialogError(""); }}>غیرفعال‌سازی</Button></div> : null}</CardContent></Card>)}</div>
    <Dialog open={editing !== null} fullScreen={mobile} fullWidth maxWidth="sm" onClose={() => !saving && setEditing(null)}><form className="admin-user-dialog-form" onSubmit={saveEdit}><DialogTitle>ویرایش {editing?.username}</DialogTitle><DialogContent dividers className="dialog-form">{dialogError ? <Alert severity="error">{dialogError}</Alert> : null}<TextField label="نام کاربری" value={editing?.username || ""} disabled helperText="نام کاربری ثابت است." /><TextField label="نام و نام خانوادگی" value={editName} onChange={(e) => setEditName(e.target.value)} required /><div className="user-permission-switches"><FormControlLabel control={<Switch checked={editSales} onChange={(e) => setEditSales(e.target.checked)} />} label="اجازه ثبت فروش" /><FormControlLabel control={<Switch checked={editCatalog} onChange={(e) => setEditCatalog(e.target.checked)} />} label="اجازه کالا، خرید و انبار" /><FormControlLabel control={<Switch checked={editLedger} onChange={(e) => setEditLedger(e.target.checked)} />} label="اجازه اشخاص و دفتر حساب" /><FormControlLabel control={<Switch checked={editChequesReports} onChange={(e) => setEditChequesReports(e.target.checked)} />} label="اجازه چک‌ها و گزارش‌ها" /></div><TextField label="دلیل تغییر" value={editReason} onChange={(e) => setEditReason(e.target.value)} required multiline minRows={2} /></DialogContent><DialogActions className="admin-user-dialog-actions"><Button onClick={() => setEditing(null)}>انصراف</Button><Button type="submit" variant="contained" disabled={saving || !editName.trim() || editReason.trim().length < 3}>ذخیره</Button></DialogActions></form></Dialog>
...
function DashboardView({
  ...
  const [snapshot, setSnapshot] = useState<{ journal: DailyJournal; inventory: InventoryReport; reminders: DueReminders } | null>(null);
  ...
  async function loadSnapshot() {
    ...
    try {
      const [journal, inventory, reminders] = await Promise.all([
        api.dailyJournal(today),
        api.inventoryReport(),
        api.dueReminders(today, jalaliDateAfterDays(7)),
      ]);
      setSnapshot({ journal, inventory, reminders });
      ...
      <div className="purchase-layout">
        <section className="sale-panel purchase-invoice-panel">
          ...
          <form className="purchase-invoice-form" onSubmit={handleSubmitPurchase} noValidate>
            ...
            <MoneyField label="پرداخت‌شده (تومان)" valueRial={paidAmount} onValueRialChange={setPaidAmount} disabled={submitStatus === "loading"} />
            <MoneyField label="تخفیف فاکتور (تومان)" valueRial={invoiceDiscount} onValueRialChange={setInvoiceDiscount} disabled={submitStatus === "loading"} />
            <MoneyField label="هزینه جانبی فاکتور (تومان)" valueRial={extraCost} onValueRialChange={setExtraCost} disabled={submitStatus === "loading"} />
  ...
  return (
    <section className="sales-workspace inventory-workspace" aria-label="انبار">
      ...
      <Dialog open={Boolean(editingItem)} onClose={() => !saving && setEditingItem(null)} fullWidth maxWidth="xs">
        ...
        <DialogContent><Stack spacing={2} sx={{ pt: 1 }}><Typography color="text.secondary">{editingItem?.variant_name}</Typography><LocalizedDecimalField autoFocus label="نقطه سفارش" value={reorderLevel} onValueChange={setReorderLevel} error={Boolean(reorderError)} helperText={reorderError || "عدد صفر یا بیشتر؛ برای حذف هشدار، خالی بگذارید."} /></Stack></DialogContent>
        ...
        <form onSubmit={submitAdjustment} noValidate>
          ...
          <DialogContent className="inventory-adjustment-content">
            ...
            <div className="inventory-adjustment-fields">
              <LocalizedDecimalField autoFocus label="مقدار" value={adjustmentQuantity} onValueChange={(value) => { setAdjustmentQuantity(value); setAdjustmentError(""); }} required disabled={adjustmentSaving} error={adjustmentQuantity.length > 0 && (adjustmentQuantityValue <= 0 || adjustmentExceedsStock)} helperText={adjustmentExceedsStock ? `مقدار واردشده از مانده ${formatDecimal(adjustingItem?.quantity_on_hand ?? 0)} بیشتر است.` : adjustmentType === "decrease" ? `حداکثر قابل کاهش: ${formatDecimal(adjustingItem?.quantity_on_hand ?? 0)}` : "عدد صحیح یا اعشاری؛ مثلاً ۱۲٫۵"} />
...
function ProductsView({ onBack, isSuperuser }: { onBack: () => void; isSuperuser: boolean }) {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down("sm"));
        ...
        <section className="unit-product-workspace" aria-label="مدیریت واحدها و کالاهای پایه">
          <section className="record-manager" aria-labelledby="unit-manager-title">
            ...
            {unitProductStatus === "ready" && filteredUnits.length > 0 ? <div className="record-list">{filteredUnits.map((unit) => (
              <article className="record-item" key={unit.id}>
                ...
                <div className="record-item-actions"><Button startIcon={<EditRounded />} onClick={() => startUnitEdit(unit)}>ویرایش</Button>{isSuperuser ? <Button color="error" startIcon={<DeleteOutlineRounded />} onClick={() => setPendingDeactivateUnit(unit)}>غیرفعال</Button> : null}</div>
              </article>
            ))}</div> : null}
          </section>
          ...
          <section className="record-manager" aria-labelledby="product-manager-title">
            ...
            {unitProductStatus === "ready" && filteredProducts.length > 0 ? <div className="record-list">{filteredProducts.map((product) => {
              const categoryName = categories.find((category) => category.id === product.category_id)?.name ?? "بدون دسته";
              return <article className="record-item record-item-product" key={product.id}><div className="admin-product-summary"><span className="admin-product-thumb">{product.image_url ? <img src={apiAssetUrl(product.image_url) ?? undefined} alt="" loading="lazy" /> : <ImageRounded />}</span><div><strong>{product.name}</strong><small>{categoryName}{product.description ? ` • ${product.description}` : ""}</small></div></div><div className="record-item-actions">{isSuperuser ? <Button startIcon={<AddPhotoAlternateRounded />} onClick={() => { setImageProduct(product); setImageFile(null); setImagePreview(""); setImageError(""); }}>{product.image_url ? "تعویض عکس" : "افزودن عکس"}</Button> : null}<Button startIcon={<EditRounded />} onClick={() => startProductEdit(product)}>ویرایش</Button>{isSuperuser ? <Button color="error" startIcon={<DeleteOutlineRounded />} onClick={() => setPendingDeactivateProduct(product)}>غیرفعال</Button> : null}</div></article>;
            })}</div> : null}
          </section>
          ...
          <section className="discount-rule-manager" aria-labelledby="discount-rule-manager-title">
            ...
            <div className="discount-rule-grid">
              <form className="discount-rule-form" onSubmit={submitPriceRule} noValidate>
                ...
                <JalaliDateField label="تاریخ شروع (اختیاری)" value={ruleStartDate} onChange={setRuleStartDate} disabled={ruleSaving} />
...
function LedgerView({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => { loadPeople(); const focusPersonId = Number(sessionStorage.getItem("store_focus_ledger_person") || 0); if (focusPersonId) { setSelectedId(focusPersonId); sessionStorage.removeItem("store_focus_ledger_person"); } }, []);
  ...
  async function submitPerson(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const payload = { name: personName.trim(), phone: toEnglishDigits(personPhone).trim() || undefined, person_type: personType, note: personNote.trim() || undefined, credit_status: personCreditStatus };
...
function ChequesView({ onBack, isSuperuser }: { onBack: () => void; isSuperuser: boolean }) {
  ...
  const [editType, setEditType] = useState<"received" | "paid">("received");
  const [editPersonId, setEditPersonId] = useState("");
  const [editBank, setEditBank] = useState("");
  const [editNumber, setEditNumber] = useState("");
  const [editAmount, setEditAmount] = useState("");
  const [editIssueDate, setEditIssueDate] = useState(currentJalaliDate);
  const [editDueDate, setEditDueDate] = useState(currentJalaliDate);
  ...
  async function load() {
    setStatus("loading");
    try { const [nextPeople, nextCheques] = await Promise.all([api.chequePersonOptions(), api.cheques()]); setPeople(nextPeople); setCheques(nextCheques); setStatus("ready"); }
  ...
  async function loadChequeAudits(chequeId: number) {
    setChequeAuditsStatus("loading");
    try { setChequeAudits(await api.chequeAudits(chequeId)); setChequeAuditsStatus("ready"); }
    catch { setChequeAuditsStatus("error"); }
  }

  function openChequeEdit(cheque: Cheque) {
    setEditCheque(cheque); setEditType(cheque.cheque_type); setEditPersonId(cheque.person_id ? String(cheque.person_id) : "");
    setEditBank(cheque.bank_name); setEditNumber(cheque.cheque_number); setEditAmount(moneyInputValue(cheque.amount_rial));
    setEditIssueDate(cheque.issue_jalali_date); setEditDueDate(cheque.due_jalali_date); setEditNote(cheque.note ?? "");
    setEditReason(""); setEditError(""); setEditStale(false); setChequeAudits([]); void loadChequeAudits(cheque.id);
  }

  async function reloadStaleCheque() {
    if (!editCheque) return;
    setEditSaving(true); setEditError("");
    try {
      const fresh = (await api.cheques()).find((item) => item.id === editCheque.id);
      if (!fresh || fresh.status !== "pending" || !fresh.is_active) { setEditCheque(null); setNotice({ type: "error", text: "این چک دیگر در انتظار نیست و امکان ویرایش ندارد." }); await load(); return; }
      openChequeEdit(fresh);
    } catch (error) { setEditError(error instanceof Error ? error.message : "نسخه جدید چک دریافت نشد."); }
    finally { setEditSaving(false); }
  }

  async function submitChequeEdit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!editCheque) return;
    const amountRial = normalizeMoney(editAmount);
    if (!editBank.trim() || !editNumber.trim() || amountRial <= 0) { setEditError("بانک، شماره چک و مبلغ مثبت الزامی هستند."); return; }
    if (editDueDate < editIssueDate) { setEditError("تاریخ سررسید نمی‌تواند پیش از تاریخ صدور باشد."); return; }
    if (!editReason.trim()) { setEditError("دلیل ویرایش را بنویسید تا تغییرات قابل پیگیری باشند."); return; }
    setEditSaving(true); setEditError(""); setEditStale(false);
    try {
      await api.updateCheque(editCheque.id, { person_id: Number(editPersonId) || null, bank_name: editBank.trim(), cheque_number: editNumber.trim(), amount_rial: amountRial, issue_jalali_date: editIssueDate, due_jalali_date: editDueDate, note: editNote.trim() || null, reason: editReason.trim(), expected_updated_at: editCheque.updated_at_utc });
      setEditCheque(null); setNotice({ type: "success", text: "اطلاعات چک ویرایش و در تاریخچه ثبت شد." }); await Promise.all([load(), loadDues()]);
    } catch (error) {
      if (error instanceof ApiError && error.status === 409) { setEditStale(true); setEditError("این چک هم‌زمان در جای دیگری تغییر کرده است. نسخه تازه را دریافت و تغییرات را دوباره بررسی کنید."); }
      else setEditError(error instanceof Error ? error.message : "ویرایش چک انجام نشد.");
    } finally { setEditSaving(false); }
  }
  ...
  return (
    <CrudWorkspace title="دفتر چک‌ها و سررسیدها" eyebrow="دریافتی و پرداختی" onBack={onBack}>
      ...
      <section className="sale-panel cheque-list-panel"><div className="ledger-section-heading"><div><strong>فهرست چک‌ها</strong><small>جست‌وجو، پیگیری و مشاهده تاریخچه</small></div><Chip label={`${filteredCheques.length.toLocaleString("fa-IR")} نتیجه`} variant="outlined" /></div><div className="cheque-filters"><TextField size="small" label="بانک، شماره یا شخص" value={search} onChange={(event) => setSearch(event.target.value)} slotProps={{ input: { startAdornment: <InputAdornment position="start"><SearchRounded /></InputAdornment> } }} /><TextField select size="small" label="نوع" value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as typeof typeFilter)}><MenuItem value="">همه</MenuItem><MenuItem value="received">دریافتی</MenuItem><MenuItem value="paid">پرداختی</MenuItem></TextField><TextField select size="small" label="وضعیت" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><MenuItem value="">همه</MenuItem><MenuItem value="pending">در انتظار</MenuItem><MenuItem value="cleared">وصول/پاس‌شده</MenuItem><MenuItem value="bounced">برگشتی</MenuItem><MenuItem value="canceled">باطل‌شده</MenuItem></TextField><TextField select size="small" label="سررسید" value={dueFilter} onChange={(event) => setDueFilter(event.target.value as typeof dueFilter)}><MenuItem value="all">همه تاریخ‌ها</MenuItem><MenuItem value="due">تا تاریخ پیگیری</MenuItem></TextField></div>{status === "loading" ? <div className="record-state"><CircularProgress size={26} /><span>در حال دریافت چک‌ها…</span></div> : null}{status === "error" ? <div className="record-state record-state-error"><span>فهرست چک‌ها دریافت نشد.</span><Button variant="outlined" startIcon={<RefreshRounded />} onClick={load}>تلاش دوباره</Button></div> : null}{status === "ready" && cheques.length === 0 ? <div className="record-state"><PaymentsRounded /><strong>هنوز چکی ثبت نشده است</strong></div> : null}{status === "ready" && cheques.length > 0 && filteredCheques.length === 0 ? <div className="record-state"><SearchRounded /><strong>چکی با این فیلتر پیدا نشد</strong></div> : null}<div className="cheque-cards">{filteredCheques.map((cheque) => { const person = people.find((item) => item.id === cheque.person_id); const statusLabel = ({ pending: "در انتظار", cleared: cheque.cheque_type === "received" ? "وصول‌شده" : "پاس‌شده", bounced: "برگشتی", canceled: "باطل‌شده" } as const)[cheque.status]; const canClear = cheque.status === "pending" || cheque.status === "bounced"; const canBounce = cheque.status === "pending"; const canCancel = cheque.status === "pending" || cheque.status === "bounced"; return <article id={`cheque-${cheque.id}`} className={`cheque-card status-${cheque.status}`} key={cheque.id}><div className="cheque-card-main"><div className="cheque-card-heading"><span className="cheque-bank-icon"><PaymentsRounded /></span><div><strong>{cheque.bank_name}</strong><small>شماره {toPersianDigits(cheque.cheque_number)}</small></div><Chip size="small" label={statusLabel} color={cheque.status === "cleared" ? "success" : cheque.status === "bounced" ? "error" : cheque.status === "pending" ? "warning" : "default"} /></div><div className="cheque-meta"><span><small>نوع</small><strong>{cheque.cheque_type === "received" ? "دریافتی" : "پرداختی"}</strong></span><span><small>شخص</small><strong>{person?.name || "ثبت نشده"}</strong></span><span><small>سررسید</small><strong>{toPersianDigits(cheque.due_jalali_date)}</strong></span></div></div><div className="cheque-card-amount"><small>مبلغ چک</small><strong>{formatRial(cheque.amount_rial)}</strong></div><div className="cheque-actions">{cheque.status === "pending" && cheque.is_active ? <Button size="large" variant="contained" startIcon={<EditRounded />} onClick={() => openChequeEdit(cheque)}>ویرایش</Button> : null}<Button size="large" variant="outlined" color="success" disabled={!canClear} onClick={() => openEvent(cheque, "cleared")}>{cheque.cheque_type === "received" ? "وصول" : "پاس"}</Button><Button size="large" variant="outlined" color="error" disabled={!canBounce} onClick={() => openEvent(cheque, "bounced")}>برگشت</Button>{isSuperuser ? <Button size="large" disabled={!canCancel} onClick={() => openEvent(cheque, "canceled")}>ابطال</Button> : null}<Button size="large" startIcon={<HistoryRounded />} onClick={() => setHistoryId(historyId === cheque.id ? null : cheque.id)}>تاریخچه</Button></div>{historyId === cheque.id ? <div className="cheque-history">{cheque.events.map((item) => <div key={item.id}><span /><p><strong>{({ created: "ثبت چک", cleared: "وصول/پاس", bounced: "برگشت", canceled: "ابطال" } as const)[item.event_type]}</strong><small>{toPersianDigits(item.jalali_date)}، ساعت {toPersianDigits(item.local_time)}{item.note ? ` • ${item.note}` : ""}</small></p></div>)}</div> : null}</article>; })}</div></section>
      <Dialog open={editCheque !== null} onClose={() => { if (!editSaving) setEditCheque(null); }} fullWidth maxWidth="md" fullScreen={isMobile} className="cheque-edit-dialog" aria-labelledby="cheque-edit-title">
        <form className="cheque-edit-dialog-form" onSubmit={submitChequeEdit} noValidate>
          <DialogTitle id="cheque-edit-title"><Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between", gap: 1 }}><span>ویرایش چک {editCheque ? toPersianDigits(editCheque.cheque_number) : ""}</span><IconButton aria-label="بستن" onClick={() => setEditCheque(null)} disabled={editSaving}><CloseRounded /></IconButton></Stack></DialogTitle>
          <DialogContent dividers className="dialog-form cheque-edit-dialog-content">
            <Alert severity="info">فقط چک فعال و در انتظار قابل ویرایش است. همه تغییرات همراه با دلیل در سابقه می‌مانند.</Alert>
            {editError ? <Alert severity="error" role="alert" action={editStale ? <Button color="inherit" onClick={() => void reloadStaleCheque()} disabled={editSaving}>دریافت نسخه تازه</Button> : undefined}>{editError}</Alert> : null}
            <div className="cheque-edit-grid"><TextField select label="نوع چک" value={editType} disabled helperText="نوع چک پس از ثبت قابل تغییر نیست."><MenuItem value="received">دریافتی</MenuItem><MenuItem value="paid">پرداختی</MenuItem></TextField><TextField select label="شخص مرتبط (اختیاری)" value={editPersonId} onChange={(event) => setEditPersonId(event.target.value)} disabled={editSaving}><MenuItem value="">بدون شخص</MenuItem>{people.map((person) => <MenuItem value={person.id} key={person.id}>{person.name}</MenuItem>)}</TextField><TextField label="نام بانک" value={editBank} onChange={(event) => { setEditBank(event.target.value); setEditError(""); }} required disabled={editSaving} error={editBank.length > 0 && !editBank.trim()} slotProps={{ htmlInput: { maxLength: 120 } }} /><TextField label="شماره چک" value={toPersianDigits(editNumber)} onChange={(event) => { setEditNumber(toEnglishDigits(event.target.value)); setEditError(""); }} required disabled={editSaving} slotProps={{ htmlInput: { dir: "ltr", inputMode: "numeric", maxLength: 80 } }} /><MoneyField label="مبلغ (تومان)" valueRial={normalizeMoney(editAmount)} onValueRialChange={(value) => { setEditAmount(moneyInputValue(value)); setEditError(""); }} required disabled={editSaving} /><JalaliDateField label="تاریخ صدور" value={editIssueDate} onChange={(value) => { setEditIssueDate(value); setEditError(""); }} required /><JalaliDateField label="تاریخ سررسید" value={editDueDate} onChange={(value) => { setEditDueDate(value); setEditError(""); }} required /><TextField className="wide" label="یادداشت (اختیاری)" value={editNote} onChange={(event) => setEditNote(event.target.value)} multiline minRows={2} disabled={editSaving} slotProps={{ htmlInput: { maxLength: 2000 } }} /><TextField className="wide" label="دلیل ویرایش" value={editReason} onChange={(event) => { setEditReason(event.target.value); setEditError(""); setEditStale(false); }} required multiline minRows={2} disabled={editSaving} error={editReason.length > 0 && !editReason.trim()} helperText="مثلاً اصلاح شماره، مبلغ یا توافق جدید درباره سررسید" slotProps={{ htmlInput: { maxLength: 2000 } }} /></div>
            {editDueDate < editIssueDate ? <Alert severity="error">تاریخ سررسید نمی‌تواند پیش از تاریخ صدور باشد.</Alert> : null}
            <section className="cheque-audit-section"><div className="ledger-section-heading"><div><strong>تاریخچه ویرایش‌ها</strong><small>مقدار قبل و بعد هر تغییر</small></div></div>{chequeAuditsStatus === "loading" ? <div className="record-state"><CircularProgress size={24} /><span>در حال دریافت تاریخچه…</span></div> : null}{chequeAuditsStatus === "error" ? <div className="record-state record-state-error"><span>تاریخچه ویرایش دریافت نشد.</span><Button variant="outlined" startIcon={<RefreshRounded />} onClick={() => editCheque && void loadChequeAudits(editCheque.id)}>تلاش دوباره</Button></div> : null}{chequeAuditsStatus === "ready" && chequeAudits.length === 0 ? <div className="record-state"><HistoryRounded /><strong>هنوز ویرایشی ثبت نشده است</strong></div> : null}{chequeAuditsStatus === "ready" && chequeAudits.length > 0 ? <div className="cheque-audit-list">{chequeAudits.map((audit) => <article key={audit.id}><header><strong>{audit.actor_full_name || audit.actor_username || "کاربر مدیریت"}</strong><small>{toPersianDigits(new Date(audit.occurred_at_utc).toLocaleString("fa-IR", { timeZone: "Asia/Tehran" }))}</small></header><div><span><small>قبل</small><strong>{audit.before_json?.bank_name || "—"} • {audit.before_json?.cheque_number ? toPersianDigits(audit.before_json.cheque_number) : "—"} • {audit.before_json?.amount_rial != null ? formatRial(audit.before_json.amount_rial) : "—"} • {audit.before_json?.due_jalali_date ? toPersianDigits(audit.before_json.due_jalali_date) : "—"}</strong></span><ArrowBackRounded /><span><small>بعد</small><strong>{audit.after_json?.bank_name || "—"} • {audit.after_json?.cheque_number ? toPersianDigits(audit.after_json.cheque_number) : "—"} • {audit.after_json?.amount_rial != null ? formatRial(audit.after_json.amount_rial) : "—"} • {audit.after_json?.due_jalali_date ? toPersianDigits(audit.after_json.due_jalali_date) : "—"}</strong></span></div><p>{audit.reason}</p></article>)}</div> : null}</section>
          </DialogContent>
          <DialogActions className="cheque-edit-dialog-actions"><Button size="large" onClick={() => setEditCheque(null)} disabled={editSaving}>انصراف</Button><Button size="large" type="submit" variant="contained" disabled={editSaving || !editBank.trim() || !editNumber.trim() || normalizeMoney(editAmount) <= 0 || editDueDate < editIssueDate || !editReason.trim() || editStale} startIcon={editSaving ? <CircularProgress size={18} color="inherit" /> : <FactCheckRounded />}>{editSaving ? "در حال ذخیره…" : "ذخیره ویرایش"}</Button></DialogActions>
        </form>
      </Dialog>
      <Dialog open={eventCheque !== null} onClose={() => { if (!eventSaving) setEventCheque(null); }} fullWidth maxWidth="xs" fullScreen={isMobile}><form onSubmit={submitEvent}><DialogTitle>{eventType === "cleared" ? (eventCheque?.cheque_type === "received" ? "ثبت وصول چک" : "ثبت پاس چک") : eventType === "bounced" ? "ثبت برگشت چک" : "ابطال چک"}</DialogTitle><DialogContent className="dialog-form"><Alert severity={eventType === "canceled" || eventType === "bounced" ? "warning" : "info"}>این اقدام در تاریخچه چک ثبت می‌شود. اطلاعات را پیش از تأیید بررسی کنید.</Alert>{eventDialogError ? <Alert severity="error">{eventDialogError}</Alert> : null}<JalaliDateField label="تاریخ اقدام" value={eventDate} onChange={(value) => { setEventDate(value); setEventDialogError(""); }} required /><TextField label="یادداشت (اختیاری)" value={eventNote} onChange={(event) => setEventNote(event.target.value)} multiline minRows={2} disabled={eventSaving} /></DialogContent><DialogActions><Button size="large" onClick={() => setEventCheque(null)} disabled={eventSaving}>انصراف</Button><Button size="large" type="submit" variant="contained" color={eventType === "cleared" ? "success" : "error"} disabled={eventSaving} startIcon={eventSaving ? <CircularProgress size={18} color="inherit" /> : <FactCheckRounded />}>تأیید و ثبت</Button></DialogActions></form></Dialog>
    </CrudWorkspace>
  );
}
...
function OnlineView({ onBack }: { onBack: () => void }) {
  ...
  async function submitRule(event: FormEvent<HTMLFormElement>) {
    ...
    const quantity = normalizeDecimal(ruleMinQuantity);
  ...
  async function submitReservation(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const quantity = normalizeDecimal(reservationQuantity);
    ...
    <CrudWorkspace title="عملیات آنلاین" eyebrow="کانال‌ها، قیمت، رزرو و سفارش" onBack={onBack}>
      ...
      {tab === 1 ? <div className="online-split">
        <form className="sale-panel online-form" onSubmit={submitRule} noValidate>
          ...
          <div className="online-form-row"><MoneyField label="قیمت آنلاین (تومان)" valueRial={normalizeMoney(rulePrice)} onValueRialChange={(value) => setRulePrice(moneyInputValue(value))} required error={rulePrice.length > 0 && normalizeMoney(rulePrice) <= 0} /><LocalizedDecimalField label="حداقل تعداد" value={ruleMinQuantity} onValueChange={setRuleMinQuantity} required error={normalizeDecimal(ruleMinQuantity) <= 0} /></div>
          ...
          <Button type="submit" variant="contained" size="large" disabled={ruleSaving || states.channels !== "ready" || states.variants !== "ready" || !ruleChannelId || !ruleVariantId || normalizeMoney(rulePrice) <= 0 || normalizeDecimal(ruleMinQuantity) <= 0 || Boolean(ruleEnds && ruleStarts > ruleEnds)} startIcon={ruleSaving ? <CircularProgress size={18} color="inherit" /> : <LocalOfferRounded />}>{ruleSaving ? "در حال ثبت…" : "ثبت قانون قیمت"}</Button>
        </form>
      ...
      {tab === 2 ? <div className="online-split">
        <form className="sale-panel online-form" onSubmit={submitReservation} noValidate>
          ...
          <div className="online-form-row"><LocalizedDecimalField label="تعداد" value={reservationQuantity} onValueChange={setReservationQuantity} required error={normalizeDecimal(reservationQuantity) <= 0} /><JalaliDateField label="تاریخ انقضا" value={reservationExpiry} onChange={setReservationExpiry} required /></div>
          ...
          <Button type="submit" variant="contained" size="large" disabled={reservationSaving || states.channels !== "ready" || states.variants !== "ready" || !reservationChannelId || !reservationVariantId || normalizeDecimal(reservationQuantity) <= 0 || reservationExpiry < currentJalaliDate()} startIcon={reservationSaving ? <CircularProgress size={18} color="inherit" /> : <Inventory2Rounded />}>{reservationSaving ? "در حال ثبت…" : "ثبت رزرو"}</Button>
        </form>
  ...
  return (
    <section className="sales-workspace" aria-label="ثبت فروش روزانه">
            ...
            <div className="payment-list">
              ...
              {payments.map((payment) => (
                <div className={`payment-row payment-${payment.status}`} key={payment.id}>
                  ...
                  <MoneyField label="مبلغ (تومان)" valueRial={payment.amountRial} onValueRialChange={(amountRial) => updatePayment(payment.id, { amountRial })} />
                  ...
                  <TextField label="شماره پیگیری (اختیاری)" value={toPersianDigits(payment.referenceNumber)} onChange={(event) => updatePayment(payment.id, { referenceNumber: toEnglishDigits(event.target.value) })} />
      ...
      <Card variant="outlined" component="section" className="journal-panel">
        ...
        {lastSale ? <Alert severity="success">آخرین فاکتور ثبت‌شده: {toPersianDigits(lastSale.invoice_number ?? lastSale.id)} • سود تخمینی {formatRial(lastSale.items.reduce((sum, item) => sum + (item.estimated_profit_rial ?? 0), 0) - lastSale.discount_amount_rial)}</Alert> : null}
