    ...
    <CrudWorkspace title="عملیات آنلاین" eyebrow="کانال‌ها، قیمت، رزرو و سفارش" onBack={onBack}>
      ...
      {tab === 1 ? <div className="online-split">
        <form className="sale-panel online-form" onSubmit={submitRule} noValidate>
          ...
          <div className="online-form-row"><MoneyField label="قیمت آنلاین (تومان)" valueRial={normalizeMoney(rulePrice)} onValueRialChange={(value) => setRulePrice(moneyInputValue(value))} required error={rulePrice.length > 0 && normalizeMoney(rulePrice) <= 0} /><TextField label="حداقل تعداد" value={toPersianDigits(ruleMinQuantity)} onChange={(event) => setRuleMinQuantity(toEnglishDigits(event.target.value))} inputMode="decimal" required error={normalizeDecimal(ruleMinQuantity) <= 0} /></div>
      ...
      {tab === 2 ? <div className="online-split">
        <form className="sale-panel online-form" onSubmit={submitReservation} noValidate>
          ...
          <div className="online-form-row"><TextField label="تعداد" value={toPersianDigits(reservationQuantity)} onChange={(event) => setReservationQuantity(toEnglishDigits(event.target.value))} inputMode="decimal" required error={normalizeDecimal(reservationQuantity) <= 0} /><JalaliDateField label="تاریخ انقضا" value={reservationExpiry} onChange={setReservationExpiry} required /></div>
